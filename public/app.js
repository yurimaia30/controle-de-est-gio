...
async function saveOp(form){
  ...
  modal(`<h2>Oportunidade salva</h2><p>${esc(student.nome)} · ${fmt(start)} a ${fmt(end)} · ${dates.length} dias de estágio.</p><p>Horas autorizadas: 0h. A ficha está pronta para download.</p>${button('Baixar ficha de frequência','pdf',ref.id,'primary')}${!external?button('Gerar TCE do campo','automatic-tce',ref.id,'primary'):''}`);notify('Oportunidade salva no Firebase. Nenhuma hora foi creditada.');
}
